...
uris = re.findall(r'data:image/([A-Za-z0-9.+-]+);base64,([A-Za-z0-9+/=]+)', html)
# index.html contains the original gallery assets plus dedicated detail-page assets.
# Keep the count check for all assets and verify both Sialk detail groups.
EXPECTED_EMBEDDED_IMAGES = 54
EXPECTED_SIALK_60120_DETAIL_IMAGES = 5
EXPECTED_SIALK_8080_DETAIL_IMAGES = 4
assert len(uris) == EXPECTED_EMBEDDED_IMAGES, len(uris)
...
for mime, payload in uris:
    assert mime.lower() == 'webp'
...
print(f'All {EXPECTED_EMBEDDED_IMAGES} embedded WebP images remain valid: OK')
