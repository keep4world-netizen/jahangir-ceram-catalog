...
`catalog-source.html` is the editable source used by the generator. The production `index.html` in this package contains the complete catalog, including its newer catalog sections. Do not regenerate it with the legacy generator unless you intentionally update that generator first; run the validator before deploying:
